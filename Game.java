...
public class Game {
    ...
    public boolean playTurnTorus(int row1,int col1,int row2, int col2 , Player player) throws IllegalArgumentException{
        ...
        return winner(player) || winTorusRow(player) || winTorusColumn(player) ;
    }
    ...
    public boolean winTorusColumn(Player player) {
    	 boolean won = false;
         for(int i = 0 ; i < board.length ; i++){
                 if((board[0][i]+board[17][i]+board[16][i]+board[15][i]+board[14][i]).equals(player.getName())){
                     won = true;
                 }
                 else if ((board[0][i]+board[1][i]+board[2][i]+board[3][i]+board[17][i])
                         .equals(player.getName()+player.getName()+player.getName()+player.getName()
                                  +player.getName()+player.getName())){
                     won = true;
                 }
